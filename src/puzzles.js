...
export class PuzzleSystem {
  ...
  _buildPatronus() {
    ...
    const { group, legs } = buildPatronusModel(this.glowTex);
    this.patronusLegs = legs;
  ...
  _updateFinale(dt) {
    ...
    this.finaleT += dt;
    // 2s nach der Fanfare: Expecto Patronum freischalten (unlockPatronum ist
    // idempotent, der Aufruf hier feuert also effektiv nur beim ersten Mal).
    if (this.finaleT >= 2) this.spells.unlockPatronum?.(true);
    if (this.finaleT < 40) {
  ...
  restore(pz = {}, artifactIds = []) {
    ...
    if (this.finaleWon) {
      this._buildPatronus();
      this.spells.unlockPatronum?.(false); // Alt-Save: sofort freigeschaltet, ohne Toast
    }
